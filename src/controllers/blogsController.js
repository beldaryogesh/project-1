...
const updateBlog = async function (req, res) {
  try {
    ...
    let data = req.body;
    if (!blogId)
      res.status(400).send({ status: false, Msg: "BlogId is not present" });
    if (!data)
      res.status(400).send({ status: false, Msg: "Input data is not found" });
...
const isDeleted = async function (req, res) {
  try {
    ...
    let updateData = await blogsModel.findByIdAndUpdate({_id:blogId},{ $and:[{isPublished:true},{isDeleted:false}]},{ $set: { isDeleted: true, deletedAt: Date.now() } },{ new: true });
...
const deleteBlogsQuery = async function (req, res) {
  try {
    ...
    let authorId = req.query.authorId;
    if (!authorId == 0)
      res.status(400).send({ status: false, Msg: "No such authors exists" });
    if (!data)
